...
struct Theme {
    #[allow(dead_code)]
    name: &'static str,
    bg: (u8, u8, u8),        // Background / field base
    panel: (u8, u8, u8),     // Panel background
    text: (u8, u8, u8),      // Primary text
    dim: (u8, u8, u8),       // Dim/secondary text
    accent: (u8, u8, u8),    // Accent color (OK status, highlights)
}
...
fn panel_bg(theme: &Theme, pulse: f64) -> Color {
    let p = theme.panel;
...
fn plasma_value(nx: f64, ny: f64, time: f64, zoom: f64) -> f64 {
    let x = nx * zoom;
    let y = ny * zoom;
    let t = time * 0.8;
...
fn plasma_color(v: f64, theme: &Theme, intensity: f64, pulse: f64) -> Color {
    let bg = theme.bg;
...
fn plasma_char(v: f64) -> char {
    const CHARS: &[char] = &[' ', '░', '▒', '▓', '█'];
    let idx = ((v + 1.0) / 2.0 * (CHARS.len() - 1) as f64).round() as usize;
    CHARS[idx.min(CHARS.len() - 1)]
}

fn plasma_effective_bg(v: f64, theme: &Theme, intensity: f64, pulse: f64) -> Color {
...
impl ShowcaseState {
    ...
    fn update(&mut self, dt: f64, render_ms: f32) {
        self.frame_count += 1;
...
fn ease_out_bounce_once(t: f64) -> f64 {
...
const VERSION_TEXT: &str = "blaeck v0.2.0";

fn render_plasma_bg(state: &ShowcaseState) -> Element {
    let time = state.time();
...
fn render_logo(state: &ShowcaseState, logo_x: f64, logo_y: f64) -> Element {
...
fn render_panel(title: &str, lines: Vec<(&str, String)>, state: &ShowcaseState) -> Element {
...
fn render_showcase(state: &ShowcaseState) -> Element {
...
fn build_showcase(state: &ShowcaseState) -> Element {
